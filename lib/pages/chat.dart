            ...
            if (snapshot.failed) {
              ...
              if (data != null && data != "") {
                ...
                for (int i = 0; i < list.length; i++) {
                  ...
                  DateTime toDisplayDateTime = DateFormat('yyyy-MM-ddTHH:mm:ss')
                  ...
                  if (((toDisplayDateTime.isBefore(now) &&
                      ...
                      ((list[i].switchManual && !list[i].isManual) ||
                          (!list[i].switchManual && !list[i].isDone)) &&
                      now.toLocal().hour < 17) {
                    index = i.toString();
                    // print('second>>>'+index);
                    break;
                ...
                if (index == "") {
                  List<TwoDLiveResult> doneList = list
                      .where((element) =>
                          ((element.switchManual && element.isManual) ||
                              (!element.switchManual && element.isDone)))
                      .toList();
                          ...
                          child: Column(
                            children: [
                              ...
                              item.set != "--" &&
                                      item.set != null &&
                                      item.set != ""
                                  ? ((item.switchManual && !item.isManual) ||
                                          (!item.switchManual && !item.isDone))
                                      ? FadeTransition(
                                          opacity: _animation,
                                          child: RichText(
                                            text: TextSpan(
